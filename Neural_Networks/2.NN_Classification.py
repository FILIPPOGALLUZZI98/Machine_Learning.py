...
# valori 0 o 1, che dipende da due variabili temperatura e tempo
# In questo caso si parla di classificazione binaria
...
file_path = datadir +'Esempio_2.csv'
df = pd.read_csv(file_path)

# Plottiamo i grafici delle variabili
features = ['Temperatura','Tempo']  ## Sono i nomi delle colonne del dataset
titles = ['Temperatura', 'Tempo']
plt.figure(figsize=(12,10))
for i, feature in enumerate(features):
    plt.subplot(2, 2, i + 1)
    plt.scatter(df[feature], df['Caffe_Buono'], s = 1, color = 'k')
    plt.title(titles[i]); plt.ylabel('Buono/Cattivo')
...
X = df[['Temperatura', 'Tempo']].values
y = df['Caffe_Buono'].values 
...
model = Sequential([
Dense(units = 20, input_shape=(X_train.shape[1],), activation='sigmoid'),
...
models_config = [
    {'layers': 2, 'units': [20, 10], 'activation': ['sigmoid', 'sigmoid']},
    {'layers': 2, 'units': [15, 10], 'activation': ['sigmoid', 'sigmoid']},
    {'layers': 3, 'units': [30, 20, 10], 'activation': ['sigmoid', 'sigmoid', 'sigmoid']},
    {'layers': 3, 'units': [10,5, 10], 'activation': ['sigmoid', 'sigmoid','sigmoid']}
]
